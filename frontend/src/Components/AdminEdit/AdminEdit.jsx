  ...
  const updateStopStatus = async (stopId, currentStatus) => {
    try {
      await axios.put(`http://localhost:4000/updatestop/${stopId}`, {
        reached: !currentStatus, // Toggle reached status
      });

      // Update the local state after the backend update is successful
      setStops((prevStops) =>
        prevStops.map((stop) =>
            ...
            stops.map((stop) => (
              <div
                ...
                <input
                  ...
                  disabled={
                    stop.reached || // Disable if already reached
                    (stops.indexOf(stop) > 0 &&
                      !stops[stops.indexOf(stop) - 1].reached) // Disable if the previous stop isn't reached
                  }
                />
                <label
                  ...
                  className={`toggle-label block w-14 h-8 rounded-full cursor-pointer relative z-10 transition-colors duration-300 ease-in-out ${
                    ...
                    stop.reached ||
                    (stops.indexOf(stop) > 0 &&
                      !stops[stops.indexOf(stop) - 1].reached)
